...
public:
    DataUntimed(std::string name) : Data(name) { /* FIXME: cannot initialize data, because of templating */ }

    // copy CTOR (DONE)
    DataUntimed(const DataUntimed & other) : Data(other) {
        _elem = other._elem;
    }
     ...
     */    
    DataUntimed* Clone() const {
        return new DataUntimed(*this); ///< call copy ctor
    }

    // implements Data::merge_in()
    virtual bool merge_in(const Data * const other ATTR_UNUSED) {
        /**
         * This class does not support merging. It could be minimu, maximum, sum, average ,...
